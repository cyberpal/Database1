...
AS 
	...
	DECLARE @MergeTableP2P TABLE (id_p2p INT);
	...
	BEGIN TRY
	   ...
	   INNER JOIN @MergeTable MgTrn
			...
			USING (SELECT id_p2p, 
                          id_cuenta_destino, 
	                      monto
					 FROM Configurations.dbo.Transaccion_P2P
                    WHERE flag_afectacion_fondeo = 0
                      AND fecha_liberacion <= @fecha_hasta) AS src 
           ON (trg.id_cuenta = src.id_cuenta_destino)	
		   WHEN MATCHED THEN 
                UPDATE SET trg.Importe_Disponibilizado = src.monto + trg.Importe_Disponibilizado
		   WHEN NOT MATCHED BY TARGET THEN 
                INSERT (id_cuenta, fecha_de_cashout, Importe_Disponibilizado) 
	            VALUES (src.id_cuenta_destino, @fecha_hasta, src.monto)
		   OUTPUT src.id_p2p
	         INTO @MergeTable;
			 
			
           UPDATE t
	          SET t.flag_afectacion_fondeo = 1,
		          t.fecha_afectacion_fondeo = GETDATE()
	         FROM Configurations.dbo.Transaccion_P2P t
	   INNER JOIN @MergeTableP2P MgTrn
		       ON t.id_p2p = MgTrn.id_p2p;
